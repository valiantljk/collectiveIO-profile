...
int main(int argc, char **argv){
  ...
  while ((c = getopt (argc, argv, "f:b:k:n:x:y:z:v:")) != -1)
    switch (c)
      {
      case 'f':
  ...
  if(mpi_rank==0) printf("sizeof(double) %d * dimx %d * dimy %d=total %.2f bytes\n",sizeof(double), dims_x, dims_y,(double)dims_x*(double)dims_y*(double)sizeof(double));
  free(data_t);
