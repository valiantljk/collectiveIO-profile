...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        $Show $F77 $PROFILE_INCPATHS      "${allargs[@]}" -I$includedir
        rc=$?
    else
        $Show $F77 $PROFILE_INCPATHS      "${allargs[@]}" -I$includedir -L$libdir $f77libs $PROFILE_PRELIB $PROFILE_FOO $rpath_flags -lmpi  $PROFILE_POSTLIB  -lrt -lgpfs -lpthread   
        rc=$?
    fi
else
    $Show $F77 $PROFILE_INCPATHS   "${allargs[@]}" -I$includedir
