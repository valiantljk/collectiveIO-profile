...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        $Show $CC   $PROFILE_INCPATHS      $allargs -I$includedir
        rc=$?
    else
        $Show $CC   $PROFILE_INCPATHS      $allargs -I$includedir -L$libdir $PROFILE_PRELIB $PROFILE_FOO $rpath_flags -lmpi  $PROFILE_POSTLIB  -lrt -lgpfs -lpthread  
        rc=$?
    fi
else
    $Show $CC   $PROFILE_INCPATHS   $allargs -I$includedir
