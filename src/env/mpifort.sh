...
if [ "$linking" = yes ] ; then
    ...
    if [ "$nativelinking" = yes ] ; then
        ...
        $Show $FC $PROFILE_INCPATHS      $allargs $FCINCDIRS $FCMODDIRS -L$libdir -lmpifort $PROFILE_PRELIB $PROFILE_FOO $rpath_flags -lmpi  $PROFILE_POSTLIB  -lrt -lgpfs -lpthread   
        rc=$?
    fi
else
    $Show $FC $PROFILE_INCPATHS   $allargs $FCINCDIRS $FCMODDIRS
    rc=$?
fi
if [ -n "$rmfiles" ] ; then
