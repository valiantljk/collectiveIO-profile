...
/* Uses a generalized version of the extended two-phase method described
	...
	*fd_end = NULL, *end_offsets = NULL;
    ...
    {
	tstart[4]=MPI_Wtime();//contiguous read
	...
	printf("not two phase collective i/o\n");
	...
	tend[4]=MPI_Wtime();// contiguous read
        printf("rank%d\tno aggregation, contiguous read time%lf\n",myrank,tend[4]-tstart[4]);
	return;
    }
    ...
    tend[5]=MPI_Wtime();
    printf("rank%d\toutlayer read+exc time%lf\n",myrank,tend[5]-tstart[5]);
...
/* calculate the number of reads of size coll_bufsize
   ...
   coll_bufsize is obtained from the hints object. */
    ...
    tstart[0]=MPI_Wtime();//total io time plus shuffle time, mainly read, partial receive.
    for (m=0; m<ntimes; m++) {
	...
	ADIOI_R_Exchange_data(fd, buf, flat_buf, offset_list, len_list,
        ...
        tend[3]=MPI_Wtime();
        printf("rank%d\titeration%d\tread time%lf\tshuffle time%lf\n",rank,m,tend[2]-tstart[2],tend[3]-tstart[3]);
	if (for_next_iter) {
...
/* nothing to send, but check for recv. */
    ...
    tend[1]=MPI_Wtime();
    printf("rank:%d\ttotal read+send+receive(p):%lf\treceive(p):%lf\n",rank,tend[0]-tstart[0],tend[1]-tstart[1]);
